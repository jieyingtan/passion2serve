...
export function buildEventInvitationEmail(input: EventInvitationEmailInput) {
  ...
  const subject = `${input.organisationName} invited you to ${input.eventName}`;
  const text = [
    `Hello ${input.participantName},`,
    "",
    `${input.organisationName} has invited you to ${input.eventName} on ${eventDate} at ${input.venue}.`,
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:620px;margin:auto;color:#173b37;line-height:1.6">
      ...
      <p><strong>${escapeEmailHtml(input.organisationName)}</strong> has invited you to take part in this event.</p>
      ...
      <p><a href="${escapeEmailHtml(input.signInUrl)}" style="display:inline-block;background:#1d7065;color:white;text-decoration:none;padding:12px 18px;border-radius:9px;font-weight:700">Sign in and confirm</a></p>
