...
export default async function ParticipantProgressPage() {
  ...
  return <div className="mx-auto max-w-6xl space-y-8">
    ...
    <section><h2 className="mb-4 text-2xl font-bold">Learning pathway</h2><div className="space-y-4">{(courses??[]).map((course,index)=>{const requirements=prerequisitesByCourse.get(course.id)??[]; const unmet=requirements.filter(id=>!completedIds.has(id)); const completed=completedIds.has(course.id); const available=!completed&&!unmet.length; const related=(events??[]).filter(event=>event.course_id===course.id); return <Card className={`border-0 ${completed?"bg-emerald-50":available?"ring-2 ring-primary/30":"opacity-80"}`} key={course.id}><CardContent className="flex gap-4 p-6"><div className={`grid size-12 shrink-0 place-items-center rounded-full ${completed?"bg-emerald-500 text-white":available?"bg-primary text-primary-foreground":"bg-muted text-muted-foreground"}`}>{completed?<Check className="size-5"/>:available?<Sparkles className="size-5"/>:<LockKeyhole className="size-5"/>}</div><div className="flex-1"><div className="flex flex-wrap items-center gap-2"><h3 className="text-lg font-bold">{index+1}. {course.name}</h3><Badge variant={completed?"success":available?"warning":"secondary"}>{completed?"Completed":available?"Eligible now":"Locked"}</Badge></div><p className="mt-1 text-sm text-muted-foreground">{course.description}</p>{requirements.length>0&&<p className="mt-3 text-sm"><strong>Requires:</strong> {requirements.map(id=>nameById.get(id)).join(", ")}{unmet.length>0&&<span className="text-destructive"> · Complete {unmet.map(id=>nameById.get(id)).join(", ")} first</span>}</p>}{related.length>0&&available&&<Button asChild className="mt-4" size="sm"><Link href="/participant/events">View related events</Link></Button>}</div></CardContent></Card>})}</div></section>
