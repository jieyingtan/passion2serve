...
export default async function LifecyclePage({params}:{params:Promise<{eventId:string}>}) {
  ...
  return <div className="mx-auto max-w-6xl space-y-7">
    <div><Link className="inline-flex items-center gap-2 text-sm font-semibold text-primary" href={stage.href}><ArrowLeft className="size-4"/>Back to {stage.label}</Link><div className="mt-5 flex flex-wrap items-end justify-between gap-4"><div><Badge>{stage.label}</Badge><h1 className="mt-2 text-3xl font-bold">{event.name}</h1><p className="mt-2 text-muted-foreground">{organisation?.name} · {formatEventDate(event.starts_at)} · {event.venue}</p></div>{status==="ongoing"&&<div className="flex flex-wrap gap-2"><Button asChild variant="outline"><Link href={`/coordinator/events/${eventId}/edit`}><Pencil className="size-4"/>Edit requirements</Link></Button><Button asChild><Link href={`/coordinator/events/${eventId}/operations`}>Manage matching</Link></Button></div>}</div></div>
