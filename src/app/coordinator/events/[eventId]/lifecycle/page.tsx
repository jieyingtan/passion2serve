...
import { ArrowLeft, CheckCircle2, Clock3, ExternalLink, History, MessageCircle, Pencil, UserCheck, Users } from "lucide-react";
...
import { attendanceAcknowledgementMessage, buildWhatsAppUrl } from "@/lib/outreach/whatsapp";
...
export default async function LifecyclePage({params}:{params:Promise<{eventId:string}>}) {
  ...
  const [
    ...
    admin.from("registrations").select("id,participant_id,status,profiles(full_name,email,phone)").eq("event_id",eventId).order("created_at"),
  ...
  return <div className="mx-auto max-w-6xl space-y-7">
    ...
    {status==="upcoming"&&<>
      ...
      <Card className="border-0"><CardHeader><WorkflowStepHeader step={2} icon={<UserCheck className="size-5 text-primary"/>} title="Participant attendance" description="Use this roster only when a participant cannot access their wallet or in-app pass." /></CardHeader><CardContent className="space-y-3">{!participantRoster?.length?<p className="rounded-lg border border-dashed p-6 text-center text-muted-foreground">No registered participants yet.</p>:participantRoster.map(item=>{const profile=related(item.profiles);const attended=attendedIds.has(item.participant_id)||item.status==="attended";const acknowledgementUrl=attended&&profile?.phone?buildWhatsAppUrl(profile.phone,attendanceAcknowledgementMessage({participantName:profile.full_name??"there",eventName:event.name})):null;return <div className="flex flex-col justify-between gap-3 rounded-xl border p-4 sm:flex-row sm:items-center" key={item.id}><div><p className="font-semibold">{profile?.full_name??"Participant"}</p><p className="text-sm text-muted-foreground">{profile?.email} · {item.status.replaceAll("_"," ")}</p></div><div className="flex flex-wrap items-center gap-2">{attended?<><Badge variant="success">Attended</Badge>{acknowledgementUrl&&<Button asChild size="sm" variant="outline"><a href={acknowledgementUrl} rel="noreferrer" target="_blank"><MessageCircle className="size-4"/>WhatsApp acknowledgement<ExternalLink className="size-3.5"/></a></Button>}</>:item.status==="no_show"?<Badge variant="secondary">No-show</Badge>:<><form action={recordManualAttendance}><input name="eventId" type="hidden" value={eventId}/><input name="participantId" type="hidden" value={item.participant_id}/><Button disabled={!['invited','registered','confirmed'].includes(item.status)} size="sm" type="submit">Mark attended</Button></form><form action={markParticipantNoShow}><input name="eventId" type="hidden" value={eventId}/><input name="participantId" type="hidden" value={item.participant_id}/><Button size="sm" type="submit" variant="ghost">No-show</Button></form></>}</div></div>})}</CardContent></Card>
