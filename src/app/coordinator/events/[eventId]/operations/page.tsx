...
import { ArrowLeft, Building2, Pencil, Sparkles, UserRoundCheck, UsersRound } from "lucide-react";
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-8">
      ...
      <section className="space-y-5">
        ...
        <div className="space-y-3">{cleanSelectedBusinesses.map((selection) => { const business = Array.isArray(selection.businesses) ? selection.businesses[0] : selection.businesses; if (!business) return null; const whatsappUrl = business.phone ? buildWhatsAppUrl(business.phone, businessOutreachMessage({ contactName: business.contact_name, eventName: event.name, organisationName: organisation?.name ?? "our beneficiary organisation", eventDate, venue: event.venue })) : null; return <Card className="border border-border/60 shadow-sm" key={selection.id}><CardContent className="grid gap-5 p-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="text-base">{business.name}</strong><Badge variant={statusVariant(selection.status)}>{selection.status.replaceAll("_", " ")}</Badge><Badge variant="outline">{selection.match_score}% match</Badge></div><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">{selection.match_explanation}</p></div><OutreachActions currentStatus={selection.status} eventId={event.id} kind="business" selectionId={selection.id} whatsappUrl={whatsappUrl} /></CardContent></Card>; })}</div>
      </section>
      ...
      <section className="space-y-5">
        ...
        <div className="space-y-3">{cleanSelectedVolunteers.map((selection) => { const volunteer = Array.isArray(selection.volunteers) ? selection.volunteers[0] : selection.volunteers; if (!volunteer) return null; const whatsappUrl = volunteer.phone ? buildWhatsAppUrl(volunteer.phone, volunteerOutreachMessage({ volunteerName: volunteer.full_name, eventName: event.name, eventDate, venue: event.venue })) : null; return <Card className="border border-border/60 shadow-sm" key={selection.id}><CardContent className="grid gap-5 p-5 md:grid-cols-[minmax(0,1fr)_auto] md:items-center"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><strong className="text-base">{volunteer.full_name}</strong><Badge variant={statusVariant(selection.status)}>{selection.status.replaceAll("_", " ")}</Badge><Badge variant="outline">{selection.match_score}% match</Badge></div><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">{selection.match_explanation}</p></div><OutreachActions currentStatus={selection.status} eventId={event.id} kind="volunteer" selectionId={selection.id} whatsappUrl={whatsappUrl} /></CardContent></Card>; })}</div>
      </section>
