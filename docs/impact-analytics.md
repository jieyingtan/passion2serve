...
| View | Assumes |
| --- | --- |
| `events_base` | `events(id, name, type, status, starts_at, ends_at, venue, organisation_id, participant_capacity, volunteer_target)` |
...
| `event_volunteers_base` | `event_volunteers(event_id, volunteer_id, assignment_status, attendance_status)` |
| `event_businesses_base` | `event_businesses(event_id, business_id, outreach_status)` |
| `certificates_base` | `certificates(event_id, participant_id)` |
| `points_base` | `points_transactions(event_id, amount, created_at)` |
