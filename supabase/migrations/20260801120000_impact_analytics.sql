...
-- Social publishing is optional today, but the analytics functions must still
-- be installable before a Postiz integration has written its first record.
...
grant select on public.social_posts, public.social_metrics to authenticated, service_role;

drop policy if exists "social_posts_coordinator_read" on public.social_posts;
create policy "social_posts_coordinator_read" on public.social_posts
for select to authenticated using (
  exists (
    select 1 from public.events e
    where e.id = social_posts.event_id
      and public.can_manage_organisation(e.organisation_id)
  )
);

drop policy if exists "social_metrics_coordinator_read" on public.social_metrics;
create policy "social_metrics_coordinator_read" on public.social_metrics
for select to authenticated using (
  exists (
    select 1
    from public.social_posts sp
    join public.events e on e.id = sp.event_id
    where sp.id = social_metrics.social_post_id
      and public.can_manage_organisation(e.organisation_id)
  )
);
...
select pt.event_id as event_id, pt.points as amount, pt.created_at as created_at
from public.point_ledger pt;

create or replace view analytics.redemptions_base with (security_invoker = on) as
select r.id as id, r.status::text as status, r.requested_at as created_at
...
as $$
  ...
  agg as (
    select
      ...
      (select count(*) from vol where assignment_status in ('confirmed', 'attended', 'no_show'))
                                                                             as confirmed,
  ...
  select
    ...
    (select count(*) from analytics.event_volunteers_base v
      where v.event_id = e.id
        and analytics.norm(v.assignment_status) in ('confirmed', 'attended', 'no_show'))
                                                                                       as volunteers_confirmed,
    (select count(*) from analytics.event_volunteers_base v
...
as $$
  ...
  base as (
    select
      org_key,
      nullif(org_key, '00000000-0000-0000-0000-000000000000'::uuid) as organisation_id,
      count(*)::bigint as events_count
    from ev
    group by org_key
  ),
  ...
  vol as (
    ...
    from analytics.event_volunteers_base v
    join ev on ev.id = v.event_id
    where analytics.norm(v.assignment_status) in ('confirmed', 'attended', 'no_show')
    group by ev.org_key
  ),
  biz as (
